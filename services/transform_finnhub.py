...
class FinnhubTransform(TableRegistrationService):
    ...
    def flatten_data(self, src_table_name: str) -> Table:
        extracted_query = f"""
            SELECT 
                ...
                t_val AS ts
            FROM `{src_table_name}`
            CROSS JOIN UNNEST(data) AS d(p_val, s_val, t_val, v_val)
            WHERE data IS NOT NULL AND CARDINALITY(data) > 0
        """
    ...
    def handle_null(self, input_table : Table) -> Table:
        ...
        handle_null_query = f"""
            SELECT 
                ...
                price, 
                volume, 
                COALESCE(trade_type, '{unknown_value}') AS trade_type,
                ts
            FROM `{temp_view_name}`
    ...
    def deduplicate_data(self, input_table: Table) -> Table:
        ...
        deduplicate_query = f"""
            ...
            FROM (
                SELECT *,
                       ROW_NUMBER() OVER (PARTITION BY symbol, price, volume ORDER BY ts DESC) AS rn
